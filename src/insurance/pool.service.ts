...
export class PoolService {
  ...
  async addCapital(
    ...
    const execute = async (client: TransactionClient) => {
      ...
      const updatedPool = await this.poolRepository.incrementCapital(
        poolId,
        amount,
        client,
      );
  ...
  async lockCapital(
    ...
    const execute = async (client: TransactionClient) => {
      ...
      const beforeState = { ...pool };
      const updatedPool = await this.poolRepository.incrementLockedCapital(
        poolId,
        amount,
        client,
      );
  ...
  async unlockCapital(
    ...
    const execute = async (client: TransactionClient) => {
      ...
      const beforeState = { ...pool };
      const updatedPool = await this.poolRepository.decrementLockedCapital(
        poolId,
        amount,
        client,
      );
